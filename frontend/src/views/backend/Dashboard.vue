  ...
  <div class="dashboard">
    ...
    <el-card class="recent-articles-card" v-loading="loading.recentArticles">
      <template #header>
        <div class="card-header">
          <h3>最近发布的文章</h3>
          <el-button type="primary" text @click="goToArticleManagement"
            >查看全部</el-button
          >
        </div>
      </template>
    ...
    <el-card class="recent-comments-card" v-loading="loading.recentComments">
      <template #header>
        <div class="card-header">
          <h3>最近评论</h3>
          <el-button type="primary" text @click="goToCommentManagement"
            >查看全部</el-button
          >
        </div>
      </template>
      ...
      >
        <el-table-column label="评论用户" width="150">
          <template #default="scope">
            <div class="user-info">
              <el-avatar :size="24" :src="getImageUrl(scope.row.userAvatar)">
                {{
    ...
    .welcome-header {
      ...
      .welcome-info {
        h2 {
          margin: 0 0 8px 0;
          font-size: 24px;
          background: linear-gradient(to right, #59a6e6, #67c23a);
