...
const vm = Vue.createApp(app).mount(''#app'');</pre>', '通过Hello World示例讲解Vue 3基础语法与数据绑定，详细介绍组件8个生命周期钩子函数，深入讲解文本插值、v-html、v-bind及JavaScript表达式，最后通过登录页面和计算属性转大写两个案例巩固知识', '/img/3.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
// 命名chunk
const Foo = () =&gt; import(/* webpackChunkName: "group-foo" */ ''./Foo.vue'')</pre>', '详解Vue 3组件化开发核心概念，包括组件注册（全局/局部）、数据传递（Props/EventBus/$emit）、Slot插槽机制（默认/具名/作用域/动态插槽），以及动态组件与异步组件的高级应用', '/img/5.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
}).mount(''#app'');</pre><p>books数组实际上是一个对象数组，其中每个对象都包含了一本书籍的详细信息。deleteItem方法用于从数组中删除指定索引的书籍。</p>', '详解Vue 3样式绑定的两种核心方式：v-bind:class的对象/数组语法及与普通class共存、组件中$attrs处理多根节点；v-bind:style的对象/数组语法及自动前缀处理；最后通过表格奇偶行样式案例综合运用所学知识', '/img/6.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
})</pre><p>该案例使用this.contents.map取代v-for指令，通过Vue.h()构造子组件的虚拟节点，实现完整的帖子列表渲染和点赞交互功能。</p>', '深入讲解Vue 3组件复用技术：DOM渲染函数与JSX语法、函数式组件context参数、模板编译原理；混入对象的全局/局部注册与合并策略；插件的install机制与使用方式；最后通过render函数渲染帖子列表的综合案例巩固知识', '/img/7.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
}</pre>', '全面讲解Vue Router路由管理：四种路由模式详解、动态路由与参数响应、嵌套路由与编程式导航、命名路由与命名视图、重定向与别名、路由组件参数传递、History模式后端配置、导航守卫完整解析流程、路由元信息与权限控制、过渡动效、数据获取两种策略、路由懒加载与代码分割、滚动行为控制', '/img/8.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
}</pre><h3>5.2 错误处理</h3><p>良好的错误处理机制可以提升用户体验：</p><ul><li>网络错误提示</li><li>请求超时处理</li><li>服务端错误码统一处理</li><li>重试机制</li></ul><h2>六、RESTful API设计规范</h2><p>前后端分离项目中，通常遵循RESTful API设计规范：</p><table border="1" cellpadding="5"><tr><th>HTTP方法</th><th>操作</th><th>示例</th></tr><tr><td>GET</td><td>获取资源</td><td>/api/users 获取用户列表</td></tr><tr><td>POST</td><td>创建资源</td><td>/api/users 创建新用户</td></tr><tr><td>PUT</td><td>更新资源（完整）</td><td>/api/users/1 更新用户1的全部信息</td></tr><tr><td>PATCH</td><td>更新资源（部分）</td><td>/api/users/1 更新用户1的部分信息</td></tr><tr><td>DELETE</td><td>删除资源</td><td>/api/users/1 删除用户1</td></tr></table>', '详解Vue 3服务端通信技术：Axios安装配置与拦截器使用、请求封装最佳实践、跨域问题解决方案（开发代理/生产Nginx/CORS）、async/await异步处理、RESTful API设计规范及错误处理机制', '/img/9.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
}</pre>', '全面介绍Vue CLI脚手架工具：安装升级与版本管理、vue serve/build/create命令使用、项目目录结构解析；深入webpack配置修改（configureWebpack/chainWebpack）、loader替换与插件配置；项目构建目标、本地预览及Nginx生产环境部署配置', '/img/10.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
...
})</pre>', '深入讲解Vuex集中式状态管理：核心概念与Store响应式原理、mapState辅助函数与对象展开运算符、Getter状态派生与mapGetters映射、Mutation同步提交与对象风格载荷、Action异步处理与Promise链式调用、模块动态注册与命名空间管理', '/img/11.jpg', 1, 1, 1145, 3, 3, 1, '2026-05-20 11:45:14', '2026-05-20 11:45:14', 0, 0);
