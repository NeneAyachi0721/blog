...
public class UserLikeService {
     ...
     * @return 是否点赞
     */
    public boolean checkUserLiked(Long articleId) {
        // 获取当前登录用户
        Long userId = JwtTokenUtils.getCurrentUser().getId();
     ...
     * @return 点赞的文章ID列表
     */
    public Long[] getUserLikedArticleIds(Long userId) {
        LambdaQueryWrapper<UserLike> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(UserLike::getUserId, userId)
     ...
     * @return 点赞文章分页数据
     */
    public Page<Map<String, Object>> getUserLikedArticlePage(Integer currentPage, Integer size) {
        // 获取当前登录用户
        Long userId = JwtTokenUtils.getCurrentUser().getId();
        ...
        // 提取文章ID列表
        List<Long> articleIds = likeList.stream()
                                     .map(UserLike::getArticleId)
        ...
        // 构建点赞ID到点赞时间的映射
        Map<Long, UserLike> likeMap = likeList.stream()
        ...
        for (Long articleId : articleIds) {
            ...
            if (article != null) {
                ...
                record.put("createTime", likeMap.get(articleId).getCreateTime());
                records.add(record);
            }
        }
